...
# We'll construct the file with proper Arabic using char codes

function A() {
    # Arabic text helper using explicit Unicode
    param([string]$s)
    return $s
}

# Part 1: DOCTYPE + CSS (all CSS is ASCII, safe)
$css = @"
<!DOCTYPE html>
...
# Part 2: HTML body with correct Arabic  
$body = @'
<body>
...
<main class="main-container">
  <header class="header-bar">
    ...
    <div class="action-controls">
      <div class="sync-status" id="gitSyncPill"><span style="display:inline-block;width:8px;height:8px;border-radius:50%;background:currentColor"></span><span id="gitSyncText">جاري التحميل</span></div>
    </div>
  </header>
  ...
  <div id="tab-dashboard" class="tab-content active">
    <div class="stats-grid">
      ...
      <div class="stat-card"><div class="stat-info"><h3>آخر مزامنة</h3><p id="lastSyncTime" style="font-size:1.1rem;font-weight:700;margin-top:5px">غير متصل</p></div><div class="stat-icon"><svg width="28" height="28" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path d="M21.5 2v6h-6M21.34 15.57a10 10 0 1 1-.57-8.38l5.67-5.67"/></svg></div></div>
    </div>
    <div class="content-box">
      <div class="box-header"><h4 class="box-title">المقالات الأخيرة المضافه</h4><button class="btn-action edit" onclick="switchTab('articles')">عرض الكل</button></div>
      <table class="articles-table"><thead><tr><th>العنوان</th><th>القسم</th><th>التاريخ</th><th>الحالة</th></tr></thead><tbody id="latestArticlesList"></tbody></table>
    </div>
  </div>

  <div id="tab-add-article" class="tab-content">
    <div class="ai-assistant-card">
      <h4 style="font-weight:800;color:var(--theme-title);margin-bottom:8px">أداة الذكاء الاصطناعي</h4>
      <p style="font-size:0.85rem;color:var(--theme-text);margin-bottom:16px">هل تواجه صعوبة؟ استخدم الذكاء الاصطناعي لمساعدتك.</p>
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;margin-bottom:16px">
        <div><label class="label-custom" style="font-size:0.8rem">اختيار المزود</label><select id="aiProvider" class="ai-select" style="width:100%" onchange="loadAIModels()"><option value="google">Google Gemini</option><option value="openrouter" selected>OpenRouter</option><option value="groq">Groq</option><option value="cerebras">Cerebras</option><option value="siliconflow">SiliconFlow</option><option value="routeway">Routeway</option><option value="github">GitHub AI</option><option value="featherless">Featherless</option><option value="opencode">OpenCode</option></select></div>
        ...
        <div><label class="label-custom" style="font-size:0.8rem">نوع النص</label><select id="aiGenType" class="ai-select" style="width:100%"><option value="long_desc">نص طويل (600-800 كلمة)</option><option value="short_desc">نص قصير (160 حرف)</option><option value="meta_desc">وصف ميتا (300 حرف)</option><option value="faq">أسئلة شائعة (JSON)</option></select></div>
      </div>
      <div><label class="label-custom" style="font-size:0.8rem">اكتب الموضوع أو الكلمة المفتاحية</label><input type="text" id="aiPromptTitle" class="input-custom" style="margin-bottom:12px" placeholder="مثال: أفضل طرق الربح"></div>
    ...
    <form id="articleForm" onsubmit="saveArticle(event)">
      ...
      <div class="content-box">
        <div class="form-grid">
          ...
          <div class="field-with-ai"><label class="label-custom" for="postSlug">الرابط المختصر (Slug)</label><div class="input-group-ai"><input type="text" id="postSlug" class="input-custom" required placeholder="example-slug"><button type="button" class="btn-ai-field" onclick="generateField('slug')">✨</button></div></div>
          <div><label class="label-custom" for="postCategory">القسم</label><select id="postCategory" class="input-custom" required></select></div>
          <div class="form-group-full field-with-ai"><label class="label-custom" for="postExcerpt">وصف المقال</label><div class="input-group-ai"><textarea id="postExcerpt" class="input-custom" rows="3" required></textarea><button type="button" class="btn-ai-field btn-ai-field-ta" onclick="generateField('excerpt')">✨</button></div></div>
          ...
          <div class="form-group-full"><label class="label-custom">تعليمات مخصصة</label><textarea id="customPrompt" class="input-custom" rows="4"></textarea><button type="button" class="btn-ai-field" onclick="generateField('custom')" style="margin-top:8px">✨ توليد بتعليمات</button></div>
          ...
          <div><label class="label-custom">خصائص SEO للصورة</label><div style="display:flex;flex-direction:column;gap:12px"><input type="text" id="imageAlt" class="input-custom" placeholder="نص بديل (alt)"><input type="text" id="imageTitle" class="input-custom" placeholder="العنوان (title)"></div></div>
          <div class="form-group-full" style="border-top:1px solid var(--theme-border);padding-top:20px;margin-top:10px"><h4 style="font-weight:800;color:var(--theme-title)">إعدادات محرك البحث (SEO)</h4></div>
          ...
          <div class="form-group-full"><h4 style="font-weight:800;color:var(--theme-title);margin-bottom:12px">أسئلة شائعة (FAQ)</h4><div id="faqContainer"></div><button type="button" class="btn-action edit" onclick="addFaqRow('','')" style="margin-top:8px">+ إضافة سؤال</button><button type="button" class="btn-ai-field" onclick="generateField('faq')" style="margin-top:8px;margin-right:8px">✨ توليد الأسئلة</button></div>
          <div class="form-group-full" style="margin-top:24px"><button type="submit" class="btn-submit">حفظ المقال ونشر</button><button type="button" class="btn-action" onclick="switchTab('dashboard')" style="margin-right:12px;padding:14px 28px">إلغاء</button></div>
        </div>
      </div>
    </form>
  </div>
  ...
  <div id="tab-articles" class="tab-content">
    <div class="content-box">
      ...
      <table class="articles-table"><thead><tr><th>العنوان</th><th>القسم</th><th>التاريخ</th><th>الحالة</th><th>الإجراءات</th></tr></thead><tbody id="allArticlesList"></tbody></table>
    </div>
  </div>
  ...
  <div id="tab-categories" class="tab-content">
    <div style="display:grid;grid-template-columns:1fr 2fr;gap:32px">
      <div class="content-box">
        ...
        <form id="categoryForm" onsubmit="saveCategory(event)">
          ...
          <div style="display:flex;flex-direction:column;gap:16px">
            ...
            <div><label class="label-custom" for="catSlug">الرابط المختصر (Slug)</label><input type="text" id="catSlug" class="input-custom" required></div>
      ...
      <div class="content-box">
        <h4 class="box-title" style="margin-bottom:24px">الأقسام الحالية</h4>
